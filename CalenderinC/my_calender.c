...
// Setting the foreground color of text for printing in a console window
void SetColor(int ForgC)
{
    ...
    {
        // Mask out all but the background attribute, and add in
...
{
    // will return the number of days in a given month
    switch (month)
    {
    case 1:
        return 31;
...
{
    // will return the name of the day
    switch (day)
    {
    case 0:
        return "Sunday";
    ...
    default:
        return "Error in getName() modeule.Invalid argument passed";
    }
}
...
{
    // returns the day number, day number since the start of the current century
...

{
    ...
    if (fp == NULL)
    {
        printf("Errpr in Opening the file");
    }
...
void printMonth(int mon, int year, int x, int y)
{
    ...
    gotoxy(8, y + 3);
    printf("Red Background indicates the Notes, Press 's' to see note:");
    ClearColor();
}
...
int main()
{
    ...
    SetConsoleTitle("Calender Project - by Aanchal");
    int choice;
    char ch = 'a';
    while (1)
    {
